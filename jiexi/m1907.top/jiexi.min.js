exports = async function (videoUrl, options) {
  ...
  const url1 = `https://z1.im1907.top/?jx=${videoUrl}`;
  const uri1 = new URL(url1);
  let response = await send(url1);
  assertOK(response);
  let html = response.body;
  let url2 = html.subRegex(/<script src="([^"]+\/top\.js)"/);

  if (!url2) {
    return null;
  }

  url2 = new URL(url2, url1).toString();
  response = await send({
    url: url2,
    headers: {
      'Referer': url1
  ...
  assertOK(response);
  html = response.body;
  const origin3 = html.subRegex(/\.src\s*=\s*"([^"]+)"\s*\+\s*location\.pathname\s*\+\s*s/);

  if (!origin3) {
    return null;
  }

  const url3 = `${origin3}${uri1.pathname}${uri1.search}`;

  function It(ee, tt, nn) {
    function i(e, t) {
  ...
  const c = new Date();
  const l = c.getTime();
  const u = 60000 * c.getTimezoneOffset();
  const m = l + u + 3600000 * 8;
  const d = new Date(m);
  let p;
  p = (p = d).getDate() + 9 + 9 ^ 10;
  p = (p = It(String(p))).substring(0, 10);
  p = It(p);
  const b = d.getDay() + 11397;
  let g = 'https://a1.m1907.top:404/api/v/?z='.concat(p, '&jx=').concat(videoUrl);
  g += '&s1ig='.concat(String(b));
  g += '&g=';
  response = await send({
    url: g,
    headers: {
  ...
  if (!data['data'] || data['type'] !== 'tv' && data['type'] !== 'movie') {
    return null;
  }

  return {
    url: data['data'][0]['source']['eps'][parseInt(data['ep']) - 1]['url']
  };
};